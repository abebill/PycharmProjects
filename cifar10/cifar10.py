...
tf.app.flags.DEFINE_string('data_dir', '/tmp/cifar10_data', '''Path to the cifar data directory''')
...
def maybe_download_and_extract():
    ...
    if not os.path.exists(filepath):
        def _progress(count, block_size, total_size):
            sys.stdout.write('\r>> Downloading %s' %(filename, float(count * block_size) / float(total_size) * 100.0))
            sys.stdout.flush()
